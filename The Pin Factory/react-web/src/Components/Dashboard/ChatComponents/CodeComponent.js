...
import { vscDarkPlus as codeStyle } from 'react-syntax-highlighter/dist/esm/styles/prism';  // Dark theme
  ...
  function getPanelTitle(props) {
    ...
    return props.title ?? languageNames[language] ?? 'Code'
  }
    ...
    return (
      <div className="group dark:bg-white/2.5">
        ...
        <div className="relative p-3">
          <SyntaxHighlighter 
                language={child.props.children.props.className?.replace('language-', '') ?? 'javascript'}
                style={{ 
                    ...codeStyle, 
                    'pre[class*="language-"]': {
                        ...codeStyle['pre[class*="language-"]'],
                        background: '#18181b',  // your background color here
                        fontSize: '12px', // your font size here
                        border: 'none',  // remove border
                        outline: 'none',  // remove outline
                        boxShadow: 'none', // remove box shadow and ring
            ...
            >
                {child.props.children.props.children ?? code}
            </SyntaxHighlighter>
          <CopyButton code={child.props.children.props.children ?? code} />
        </div>
      </div>
    )
  }
  
  function CodeGroupHeader({ title, languages, children, selectedIndex }) {
    ...
    return (
        <div>
            <CodeGroupContext.Provider value={true}>
                <Container
                {...containerProps}
                className="not-prose my-6 overflow-hidden rounded-lg bg-zinc-900 shadow-md dark:ring-1 dark:ring-white/10"
                >
                    {/* Header of whole codebox */}

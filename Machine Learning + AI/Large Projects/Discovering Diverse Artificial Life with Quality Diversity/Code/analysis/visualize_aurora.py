...
def visualize_aurora(run_dir):
	...
	with open("descriptors_3d.json", "w") as f:
		json.dump(descriptors_3d.tolist(), f)

	fig, _ = plot_aurora_repertoire(config, repertoire)
	fig.savefig(run_dir / "repertoire.pdf", bbox_inches="tight")
	plt.show()
	plt.close()
	...
	for i, genotypes in enumerate(jnp.split(repertoire.genotypes, repertoire.size // MAX_GENOTYPES)):
		...
		for j in range(MAX_GENOTYPES):
			...
			# Video
			mediapy.write_video(media_dir / f"video_{genotype_index:04d}.mp4", accum.phenotype[j], fps=50)

			# Phenotype small
			mediapy.write_image(media_dir / f"phenotype_small_{genotype_index:04d}.png", accum_small.phenotype[j, -1])

			# Phenotype medium
			mediapy.write_image(media_dir / f"phenotype_medium_{genotype_index:04d}.png", accum_medium.phenotype[j, -1])

			# Phenotype large
			mediapy.write_image(media_dir / f"phenotype_large_{genotype_index:04d}.png", accum_large.phenotype[j, -1])

			# VAE
			mediapy.write_image(media_dir / f"vae_{genotype_index:04d}.png", phenotype_recon[j, -1])


if __name__ == "__main__":
	run_dirs = [
		"/workspace/src/output/aurora/pos_linear_velocity_avg/2024-06-05_224145_296752",
	]

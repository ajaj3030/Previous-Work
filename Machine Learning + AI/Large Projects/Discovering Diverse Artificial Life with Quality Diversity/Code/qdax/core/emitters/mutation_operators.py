...
def n_isoline_variation(
    ...
    def _variation_fn(
        ...
        iso_noise = -abs(jax.random.normal(random_key, shape=x1.shape) * iso_sigma)
        x = (x1 + iso_noise) + jax.vmap(jnp.multiply)((x2 - x1), line_noise)
...
def temporal_momentum_variation(
    x1: Genotype,
    x2: Genotype,
    random_key: RNGKey,
    momentum_strength: float,
    noise_strength: float,
    historical_changes: List[Genotype],
    ...
    momentum = compute_momentum(historical_changes[-history_length:])
    
    random_key, subkey1, subkey2 = jax.random.split(random_key, 3)
    
    # Combine parents
    offspring = jax.tree_map(lambda a, b: (a + b) / 2, x1, x2)
    
    # Apply momentum
    offspring = jax.tree_map(lambda x, m: x + momentum_strength * m, offspring, momentum)
    
    # Apply noise
    noise = jax.tree_map(lambda x: jax.random.normal(subkey1, x.shape) * noise_strength, offspring)
    offspring = jax.tree_map(lambda x, n: x + n, offspring, noise)

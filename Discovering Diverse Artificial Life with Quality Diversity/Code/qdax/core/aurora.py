...
from qdax.core.emitters.standard_emitters import AdaptiveMixingEmitter
...
class AdaptiveAURORA:
    def __init__(
        self,
        emitter: AdaptiveMixingEmitter,
        scoring_fn: Callable,
        descriptor_fn: Callable,
        fitness_fn: Callable,
        train_fn: Callable,
    ...
    def train(
        ...
        train_state: Any,
        random_key: RNGKey,
    ) -> Tuple[UnstructuredRepertoire, Any, Any]:
        key, subkey = jax.random.split(random_key)
        train_state, metrics = self._train_fn(subkey, repertoire, train_state)

        # New fitnesses
        keys = jax.random.split(key, repertoire.size)
        new_fitnesses = jax.vmap(self._fitness_fn, in_axes=(0, None, 0))(
            repertoire.observations, train_state, keys
        )
        new_fitnesses = jnp.where(repertoire.fitnesses == -jnp.inf, -jnp.inf, new_fitnesses)

        # New descriptors
        keys = jax.random.split(key, repertoire.size)
        new_descriptors = jax.vmap(self._descriptor_fn, in_axes=(0, None, 0))(
            repertoire.observations, train_state, keys
        )
        new_descriptors = jnp.where(
            (repertoire.fitnesses == -jnp.inf)[..., None], jnp.nan, new_descriptors
        )
    ...
    def init(
        ...
        fitnesses, descriptors, extra_scores, random_key = self._scoring_fn(
            init_genotypes, train_state, random_key,
        )
        observations = extra_scores["observations"]
    ...
    def update(
        ...
        random_key: RNGKey,
        train_state: Any,
    ) -> Tuple[UnstructuredRepertoire, Optional[EmitterState], Metrics, RNGKey]:
        genotypes, random_key = self._emitter.emit(repertoire, emitter_state, random_key)

        fitnesses, descriptors, extra_scores, random_key = self._scoring_fn(
            genotypes, train_state, random_key,
        )
        observations = extra_scores["observations"]
        ...
        metrics = self._metrics_fn(repertoire)
        metrics["is_offspring_added"] = is_offspring_added
        metrics["batch_size"] = self._emitter.batch_size

...
def visualize_me(run_dir):
	...
	# Lenia steps
	lenia_step = partial(lenia.step, observation_size=config_lenia.world_size, center_observation=False, record_observation=True)
	lenia_step_large = partial(lenia.step, observation_size=config.world_size, center_observation=True, record_observation=True)
	...
	for i, genotypes in enumerate(jnp.split(repertoire.genotypes, config.qd.repertoire_size // MAX_GENOTYPES)):
		...
		for j in range(MAX_GENOTYPES):
			...
			# Video
			mediapy.write_video(media_dir / f"video_{genotype_index:04d}.mp4", accum.observation[j], fps=50)

			# Phenotype large
			mediapy.write_image(media_dir / f"phenotype_large_{genotype_index:04d}.png", accum_large.observation[j, -1])


if __name__ == "__main__":
	run_dirs = [
		"/workspace/src/output/me/neg_angle_var/2024-06-04_115049_932716",
	]

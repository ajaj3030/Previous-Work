...
from cl_vae import loss as loss
from qdax.core.aurora import AURORA
from qdax.core.emitters.mutation_operators import isoline_variation
from qdax.core.emitters.standard_emitters import MixingEmitter
from qdax.core.emitters.standard_emitters import GaussianMutationEmitter
...
@hydra.main(version_base=None, config_path="configs/", config_name="aurora_cl")
def main(config: DictConfig) -> None:
	...
	wandb.init(project="Thesis", name=f"AURORA-ContrastiveLearning-{currentdatetime}-{config.pattern_id}", config=OmegaConf.to_container(config, resolve=True))
	...
	config_lenia = ConfigLenia(
		# Init pattern
		# if pattern_id is random, then a random pattern is generated
	...
	contrastive_loss_flag = config.qd.contrastive_loss_fn
	...
	def calculate_rotational_diversity(repertoire):
		...
		def get_top_k_average(row):
			...
			return jnp.mean(sorted_row[1:k+1])  # Exclude the first one as it's the self-comparison (0)
	...
	if config.qd.use_data_augmentation:
		def data_augmentation(batch, key):
			...
			return original_batch, aug_batch
	else:
		def data_augmentation(batch, key):
			return batch
	...
	@partial(jax.jit, static_argnames=("learning_rate_fn", "contrastive_loss_flag"))
	def train_step(train_state, batch, aug, key, learning_rate_fn, contrastive_loss_flag, beta=config.qd.beta, gamma=config.qd.gamma, delta=config.qd.delta, temperature=config.qd.temperature, margin=config.qd.margin):
		def loss_fn(params):
			# Split the key for different uses
			key1, key2, key3 = jax.random.split(key, 3)
			
			logits, mean, logvar = train_state.apply_fn(params, batch, key1)
			aug_logits, aug_mean, aug_logvar = train_state.apply_fn(params, aug, key2)
			
			loss_, (recon_loss, kld_loss, cont_loss) = loss(
				logits, batch, mean, logvar, aug_mean, contrastive_loss_flag, 
				key3, beta, gamma, delta, temperature, margin
			)
			
			logging.info(f" Recon Loss: {recon_loss}, KLD Loss: {kld_loss}, Cont Loss: {cont_loss}")
			return loss_
		...
		return train_state, {"loss": loss_value, "learning_rate": learning_rate}
	...
	def train_epoch(train_state, repertoire, key):
		...
		def scan_train_step(carry, x):
			...
			if config.qd.use_data_augmentation:
				batch, aug = data_augmentation(batch, subkey_2)
			else:
				aug = batch
			train_state, metrics = train_step(train_state, batch, aug, subkey_3, learning_rate_fn, contrastive_loss_flag)
	...
	metrics = dict.fromkeys(["generation", "qd_score", "coverage", "max_fitness", "loss", "learning_rate", "n_elites", "variance", "time", "Rotational Similarity Score", "Rotational Diversity Score", "Unique Patterns"], jnp.array([]))
	...
	def aurora_scan(carry, unused):
		...
		# Split the key for calculate_rotational_diversity
		key, subkey = jax.random.split(key)
	...
	for generation in range(0, config.qd.n_generations, config.qd.log_interval):
		...
		wandb.log(log_metrics)  # Add this line to log to wandb
